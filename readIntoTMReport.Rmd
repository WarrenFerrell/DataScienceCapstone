...
## Model
Generate models by traversing each line in file. Use a list of linked lists (essentially 
...
```{r gdmModel}
#rm(list=c("tdm.trim", "corpus","termFreq"))
source('C:/Users/warre/Dropbox/GitHub/CAPSTONE/scripts/gdmModel.R')
modelConststr <- compiler::cmpfun( modelConststr )
...
```{r makeModel}
fileName <- paste0(filePre, "gramModel", "MinFreq", minFreq)
if ( file.exists(fileName) && cacheObj ) {
    load(file = fileName)
} else {
    gramModel <- nGramModel.create(gramFreq, 10)
