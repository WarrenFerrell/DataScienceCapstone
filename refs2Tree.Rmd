...
objPre <- paste0(objPath, 'refs/inPlace/', xChars, 'Vocab', vocabulary,
                 'clnFrq', cleanFreq, 'nTpGrms', nTopGrams,'mxGrm', maxGram)
...
topGrams <- function(){ readRefData(trainPath, includedTerms, vocabulary, cleanFreq, NULL,
                                     nTopGrams, maxGram, xChars, FALSE, FALSE) }
gramFreq <- function(){ readRefData(trainPath, includedTerms, vocabulary, cleanFreq, minFreq,
                                     NULL, maxGram, xChars, FALSE, FALSE) }

Rprof('topGrams.out')
system.time({ gramTreeTop <-  topGrams() }) #faster than foreach package
Rprof(NULL)
system.time({gListTop <- env2list(gramTreeTop$tree)})
system.time({gListTopNamed <- env2Namedlist(gramTreeTop$tree, includedTerms)})
Rprof('gramFreq.out')
system.time({ gramTreeFreq <-  gramFreq() })
Rprof(NULL)
system.time({gListFreq <- env2list(gramTreeFreq$tree)})
system.time({gListFreqNamed <- env2Namedlist(gramTreeFreq$tree, includedTerms)})

# summaryRprof('treeMake.out')
summaryRprof('topGrams.out')
summaryRprof('gramFreq.out')
proftable('topGrams.out')
proftable('gramFreq.out')
microbenchmark(topGrams(), gramFreq(), times = 2)
...
fileName <- paste0(objPre, 'gList', 'nTpGrms', nTopGrams) 
# Rprof('listMake.out')
...
```{r playingWithParallel, include = FALSE}
# noFor <- function(){ readRefData(trainPath, includedTerms, vocabulary, cleanFreq,
#                                      nTopGrams, maxGram, xChars, FALSE, FALSE) }
# forDo <- function(){ readRefData(trainPath, includedTerms, vocabulary, cleanFreq,
#                                      nTopGrams, maxGram, xChars, TRUE, FALSE) }
# forPr <- function(){ readRefData(trainPath, includedTerms, vocabulary, cleanFreq,
#                                      nTopGrams, maxGram, xChars, TRUE, TRUE) }
```
