...
if ( cacheObj && file.exists(fileName) ) {
    ...
    system.time({gramTree <- readRefData(trainPath, includedTerms, maxGram, maxSize, minFreq, FALSE)})
    
    # Rprof(NULL)
    
    #system.time(save(gramTree, file = fileName)) #faster and smaller to change to List and then save
}
...
system.time({gList <- env2list(gramTree$tree)})
system.time({save(gList, file = paste0(objPre, 'gList', 'MinFreq', minFreq))})
