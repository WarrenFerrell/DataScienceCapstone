...
vocabulary = 20000
xChars = 5E7 # work with subset of data from each set
...
minFreq <- 2
maxGram <- 10
maxSize <-  1000  # limit of model in MB : memory.limit() - mem_used() 
...
if ( cacheObj && file.exists(fileName) ) {
    ...
    # Rprof('read.out')
    gramModel <- readRefData(trainPath, includedTerms, maxGram, maxSize, minFreq, FALSE)
    # Rprof(NULL)
